  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className={`w-full px-4 py-3 rounded-xl text-sm text-left flex items-center justify-between gap-2 border transition-all duration-200 ${
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-10">
        ...
        <div className={`flex items-center justify-between mb-6 ${mounted ? "fade-up" : "opacity-0"}`}>
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-200 hover:-translate-y-1 active:translate-y-0 active:scale-95 ${
                    isActive
        ...
        <div className={`mb-4 ${mounted ? "fade-up" : "opacity-0"}`}>
          <div className="relative">
            ...
            <button
              ...
              className={`absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1.5 rounded-lg text-sm flex items-center gap-1.5 transition-all ${showFilters ? "bg-violet-600/30 text-violet-300 border border-violet-500/30" : "bg-white/[0.05] hover:bg-white/[0.1] text-white/60 border border-transparent"}`}
            >
        ...
        {/* Panneau filtres */}
        {showFilters && (
          <div className="mb-6 p-5 rounded-2xl bg-white/[0.03] border border-white/[0.07] backdrop-blur-sm fade-up">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              ...
              <div className="md:col-span-2 lg:col-span-4">
                ...
                <div className="flex gap-2 flex-wrap items-center">
                  {[{ value: "recent", label: "📅 Plus récent" }, { value: "prix_asc", label: "💰 Prix croissant" }, { value: "prix_desc", label: "💰 Prix décroissant" }].map(opt => (
                    <button key={opt.value} onClick={() => setFilters({...filters, tri: opt.value})}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filters.tri === opt.value ? "bg-violet-600 text-white" : "bg-white/[0.05] text-white/50 hover:bg-white/[0.08]"}`}>
                      {opt.label}
                    </button>
                  ))}
                  <button onClick={() => setFilters({ prixMin: "", prixMax: "", wilaya: "", date: "", tri: "recent", categorie: "" })}
                    className="ml-auto px-3 py-1.5 rounded-lg text-xs font-medium bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all">
                    Réinitialiser
                  </button>
                </div>
              </div>
        ...
        {showForm && (
          <div className="mb-8 p-6 rounded-2xl bg-white/[0.03] border border-white/[0.08] backdrop-blur-xl shadow-[0_0_0_1px_rgba(139,92,246,0.1)] fade-up">
            <h2 className="text-lg font-bold mb-5">{activeTab === "annonces" ? "Nouvelle annonce" : "Nouveau service"}</h2>
            <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                ...
                <label className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] border-dashed cursor-pointer hover:border-violet-500/40 transition-all">
                  <span className="text-white/40 text-sm">📷 {form.photo ? form.photo.name : "Choisir une photo"}</span>
        ...
        {loadingItems ? (
          ...
          <div className="text-center py-20 text-white/30">
            ...
            <p className="text-lg font-medium">{activeTab === "annonces" ? "Aucune annonce" : "Aucun service"} pour le moment</p>
            <p className="text-sm mt-1">{user ? `Soyez le premier à publier !` : "Connectez-vous pour publier"}</p>
          </div>
        ) : (
            ...
            {currentItems.map((item, i) => (
              ...
              >
                {item.photo && item.photo.startsWith("http") ? (
                  <img src={item.photo} alt={item.titre} className="w-full h-44 object-cover" />
                ) : (
                  <div className="w-full h-44 bg-gradient-to-br from-violet-900/30 to-pink-900/20 flex items-center justify-center">
                    <span className="text-4xl opacity-30">{activeTab === "annonces" ? "🖼️" : "🛠️"}</span>
                  </div>
                )}

                <div className="p-4">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <span className="text-[11px] px-2 py-0.5 rounded-full bg-violet-500/15 text-violet-300 font-medium">{item.categorie}</span>
                    <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${item.role === "technicien" ? "bg-blue-500/15 text-blue-300" : "bg-pink-500/15 text-pink-300"}`}>
                      {item.role === "technicien" ? "👨‍🔧 Pro" : "👤 Particulier"}
                    </span>
                  </div>
                  <h3 className="font-bold text-base text-white mb-1 line-clamp-1">{item.titre}</h3>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-white/25">👤 {item.auteur}</span>
                  </div>
                  {item.wilaya && <p className="text-xs text-white/30 mt-2">📍 {item.wilaya}</p>}
